...
export default function TableauResultats({ titre, data }) {
  ...
  const testGlobal =
    etatTheo === 'Excédent' && etatPrat === 'Excédent' ? 'Excédent' : 'Dépassement';
  ...
  return (
    <div className="bg-white shadow rounded-2xl p-4 mb-8">
      ...
      <table className="w-full table-auto border text-sm mb-4">
        ...
        <tbody>
          <tr>
            <td className="border p-2">Salles Théoriques</td>
            ...
            <td
              className={`border p-2 text-center font-semibold ${
                etatTheo === 'Excédent' ? 'text-green-600' : 'text-red-600'
              }`}
            >
              {etatTheo}
            </td>
